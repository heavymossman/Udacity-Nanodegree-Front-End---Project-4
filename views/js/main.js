...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        document.getElementById("pizzaSize").innerHTML = "Small"; // Added in document.getElementById which is quicker than query all selector, but this didn't make much difference! 
        return;
      case "2":
  ...
  function determineDx (elem, size) {
    ...
    var windowWidth = document.getElementById("randomPizzas").offsetWidth; //change this to getElementbyID
...
//The switch style is much faster than the olf for loop. Had to check forum to get this to work perfectly. 
 ...
 function changePizzaSizes(size) {
    ...
    var randomPizzas = document.getElementsByClassName(".randomPizzaContainer"); //Changed this to element by class name to further improve performance! 
...
// Moves the sliding background pizzas based on scroll position - AND OPTIMSE THIS FUNCTION
function updatePositions() { // OPTIMSE THIS
  frame++;
  window.performance.mark("mark_start_frame");

  var xStart = document.body.scrollTop / 1250; //pulled out position starting to improve load times, no need to do it each iteration


  var items = document.getElementsByClassName('mover'); //MAYBE CHANGE THIS? - YES Use Class Names, thanks help video!!
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(xStart + (i % 5)); //updated this line with xStart rather than the original start position
...
// runs updatePositions on scroll //OPTIMSE THIS FUNCTION
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Rather than generate 200 pizzas for every page width, going to use a variable screen sized function to create dynamic number of pizzas

    var randomPizzasGen = 0; 

    console.log("Total number of pizzas: " + randomPizzasGen);

    var windowSize = window.innerWidth; 

    console.log(windowSize) //working

    //Sizes from bootstrap media things
    ...
    if (windowSize >= 1200) {
      ...
      randomPizzasGen = 10; //mobile
    }

    console.log("Total number of pizzas: " + randomPizzasGen);


  for (var i = 0; i < randomPizzasGen; i++) {
